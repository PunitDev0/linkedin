...
import { Facebook, Twitter, Github, Moon, Sun } from 'lucide-react'
      ...
      <div
          ...
          {!isMobile && (
            <motion.div
              ...
              <motion.div
                ...
                transition={{ delay: 0.2, duration: 0.5 }}
                className="absolute top-20 left-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 w-40 md:w-48">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Total Growth</span>
                  <span className="text-xs md:text-sm text-green-500">+16%</span>
                </div>
                <div className="text-xl md:text-2xl font-bold dark:text-white">$27.9k</div>
                <div
                  className="mt-2 relative h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: '75%' }}
                ...
                <div
                  className="mt-1 text-center text-xs md:text-sm font-medium dark:text-gray-300">12%</div>
              </motion.div>
              <motion.div
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.4, duration: 0.5 }}
                className="absolute top-20 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 w-40 md:w-48">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Total Profit</span>
                  <span className="text-xs md:text-sm text-red-500">-18%</span>
                </div>
                <div className="text-xl md:text-2xl font-bold dark:text-white">$89.34k</div>
                <div className="mt-1 text-xs md:text-sm text-gray-500 dark:text-gray-400">Last One Year</div>
              </motion.div>
              <motion.div
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.6, duration: 0.5 }}
                className="absolute bottom-20 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 w-40 md:w-48">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Total Revenue</span>
                  <span className="text-xs md:text-sm text-green-500">+22%</span>
                </div>
                <div className="text-xl md:text-2xl font-bold dark:text-white">$48.2k</div>
              </motion.div>
              <motion.div
                initial={{ scale: 0, rotate: -180 }}
                animate={{ scale: 1, rotate: 0 }}
                transition={{ delay: 0.8, duration: 0.5, type: 'spring', stiffness: 200 }}
                className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-16 h-16 bg-purple-500 rounded-lg" />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div
          ...
          <motion.h2
            ...
            {isSignUp ? "Adventure starts here 🚀" : "Welcome back 👋"}
          </motion.h2>
          <motion.p variants={itemVariants} className="text-gray-500 dark:text-gray-400 mb-6">
